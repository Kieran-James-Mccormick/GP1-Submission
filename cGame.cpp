...
void cGame::initialise(SDL_Window* theSDLWND, SDL_Renderer* theRenderer)
{
	...
	theAreaClicked = { 0, 0 };
	// Store the textures
	textureName = { "Background", "building", "player","Police", "OpeningScreen", "ClosingScreen", "HScoreScreen"};
	texturesToUse = { "Images/Sprites/Bkgnd64x64.png", "Images/Sprites/player64x64.png", "Images/Sprites/police64x64.png","Images/Bkg/Bkgnd.png", "Images/Bkg/OpeningScreenF.png", "Images/Bkg/ClosingScreenF.png","Images/Bkg/BkgndHS.png" };
	...
	// Create textures for Game Dialogue 
	...
	gameTextNames = { "TitleTxt", "CollectTxt", "InstructTxt", "ThanksTxt", "SeeYouTxt", "HSTable", "HScore"};
	gameTextList = { "Car chase", "avoid the police for as long as possible", "Use the arrow keys to navigate the map.", "Thanks for playing!", "See you again soon!", "score: ", "", "High Score"};
	for (unsigned int text = 0; text < gameTextNames.size(); text++)
	{
		if (text == 0 || text == gameTextNames.size()-1)
		{
			theTextureMgr->addTexture(gameTextNames[text], theFontMgr->getFont("calibri")->createTextTexture(theRenderer, gameTextList[text], textType::solid, { 44, 203, 112, 255 }, { 0, 0, 0, 0 }));
		}																									
		else
		{
			theTextureMgr->addTexture(gameTextNames[text], theFontMgr->getFont("calibri")->createTextTexture(theRenderer, gameTextList[text], textType::solid, { 44, 203, 112, 255 }, { 0, 0, 0, 0 }));
	...
	for (int item = 0; item < theHSTableSize; item++)
	{
		...
		theTextureMgr->addTexture(highScoreTextures[item], theFontMgr->getFont("calibri")->createTextTexture(theRenderer, entry.c_str(), textType::solid, { 44, 203, 112, 255 }, { 0, 0, 0, 0 }));
 	}
...
void cGame::render(SDL_Window* theSDLWND, SDL_Renderer* theRenderer)
{
	...
	case gameState::playing:
	{
		...
		pos = { 10, 10, tempTextTexture->getTextureRect().w, tempTextTexture->getTextureRect().h };
		tempTextTexture->renderTexture(theRenderer, tempTextTexture->getTexture(), &tempTextTexture->getTextureRect(), &pos, scale);
		theTextureMgr->addTexture("score", theFontMgr->getFont("calibri")->createTextTexture(theRenderer, strScore.c_str(), textType::solid, { 44, 203, 112, 255 }, { 0, 0, 0, 0 }));
...
void cGame::update(double deltaTime)
{
	...
	if (theGameState == gameState::menu)
	{
		...
		{
/*spawns the police and player in*/
			theTileMap.update(thePlayer.getMapPosition(), 1, 0.0f);
			theTileMap.update(thePolice.getMapPosition(), 1, 0.0f);
			thePlayer.setMapPosition(3,0);
			thePolice.setMapPosition(0,0);

/* renders the player on the map*/
			theTileMap.update(thePlayer.getMapPosition(), 8, thePlayer.getPlayerRotation());

/* renders the police on the map*/
			theTileMap.update(thePolice.getMapPosition(), 7, thePolice.getSpriteRotAngle());
		int	score = 0;
		}
	}
	...
	if (theGameState == gameState::playing)
	{
		...
		// Check if player has chrashed or been caught
		if (thePlayer.getMapPosition() == thePolice.getMapPosition() /*|| thePlayer.getMapPosition() == thebuilding.getMapPosition())*/)
		{
			gameOver = true;
		}


		// Check if Police have caught the player
		if (thePlayer.getMapPosition() == thePolice.getMapPosition())
		{
			gameOver = true;
		}
			...
			case SDL_KEYDOWN:
				...
				case SDLK_DOWN:
				{
					if (theGameState == gameState::playing && thePlayer.getMapPosition().R < 9)
					{
						...
						theTileMap.update(thePolice.getMapPosition(), 1, 0.0f);
						thePolice.movePolice(thePlayer.getMapPosition().R, thePolice.getMapPosition().C);
						theTileMap.update(thePolice.getMapPosition(), 4, thePolice.getSpriteRotAngle());
						score++;
					}
				}
				break;
				...
				case SDLK_UP:
				{
					...
					{
						thePlayer.setPlayerRotation(180.0f);
						theTileMap.update(thePlayer.getMapPosition(), 1, 0.0f);
						thePlayer.update(thePlayer.getMapPosition().C, thePlayer.getMapPosition().R - 1);
						theTileMap.update(thePlayer.getMapPosition(), 3, thePlayer.getPlayerRotation());
						theTileMap.update(thePolice.getMapPosition(), 1, 0.0f);
				...
				case SDLK_RIGHT:
				{
					if (theGameState == gameState::playing && thePlayer.getMapPosition().C < 9)
					{
/*moves the player right by one tile*/
						thePlayer.setPlayerRotation(270.0f);
						theTileMap.update(thePlayer.getMapPosition(), 1, 0.0f);
						thePlayer.update(thePlayer.getMapPosition().C + 1, thePlayer.getMapPosition().R);
						theTileMap.update(thePlayer.getMapPosition(), 3, thePlayer.getPlayerRotation());
/* moves the police towards the player when the player moves*/
						theTileMap.update(thePolice.getMapPosition(), 1, 0.0f);
						thePolice.movePolice(thePolice.getMapPosition().R, thePlayer.getMapPosition().C);
						theTileMap.update(thePolice.getMapPosition(), 4, thePolice.getSpriteRotAngle());
						score++;
					}
				}
				...
				case SDLK_LEFT:
				{
					if (theGameState == gameState::playing && thePlayer.getMapPosition().C > 0)
					{
/*moves the player left by one tile*/
						thePlayer.setPlayerRotation(90.0f);
						theTileMap.update(thePlayer.getMapPosition(), 1, 0.0f);
						thePlayer.update(thePlayer.getMapPosition().C - 1, thePlayer.getMapPosition().R);
						theTileMap.update(thePlayer.getMapPosition(), 3, thePlayer.getPlayerRotation());
/* moves the police towards the player when the player moves*/
